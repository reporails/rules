...
## Check 6: Source Admission Validation

For each source in sources.yml:

```
1. weight must be >= 0.4 (admission threshold)
...
**Error:** `E4010: Source "{source}" has weight {weight} below admission threshold 0.4`
**Error:** `E4011: Source "{source}" type "{type}" doesn't match weight {weight}`
**Warning:** `W4002: Source "{source}" has no claims`
...
Validation:
  ✓ backed_by references: 156/156 resolved
  ✓ Bidirectional links: 156/156 consistent
  ✗ Confidence alignment: 1 issue
  ⚠ Orphaned claims: 3 warnings

Issues:
  E4007: C6 has confidence:high but source weight is 1.0 (OK)
  W4001: keepachangelog.unreleased-section has no rules
